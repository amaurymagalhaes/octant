...
user_antisybil_status_model = api.model(
    "UserAntisybilStatus",
    {
        "status": fields.String(
            required=True, description="Unknown or Pending or Known"
        ),
        "score": fields.String(
            required=False,
            description="Score",
...
class AntisybilStatus(OctantResource):
    ...
    def get(self, user_address: str):
        ...
        return {"status": antisybil_status}, 200

    @ns.doc(
        description="Refresh cached antisybil status",
    )
    @ns.marshal_with(user_antisybil_status_model)
    @ns.response(202, "Refresh in progress")
    ...
    def put(self, user_address: str):
        app.logger.info(f"Updating user {user_address} patron mode status")
        status, score, expires_at = update_user_antisybil_status(user_address)
        app.logger.info(
            f"User {user_address} antisybil status refreshed {[status, score]}"
        )

        return {"status": status, "score": score}, 204
